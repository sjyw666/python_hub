...
# 生成自定义的时间日期序列
timestamps = [start_date + i * time_interval for i in range(len(water_level))]

# 打印最优分割点及对应的时间日期
best_split_timestamp_water = timestamps[best_split_point_water]
best_split_timestamp_flow = timestamps[best_split_point_flow]
best_split_timestamp_sediment = timestamps[best_split_point_sediment]

print("Best split point for water level:", best_split_point_water)
print("Corresponding timestamp:", best_split_timestamp_water)

print("Best split point for flow rate:", best_split_point_flow)
print("Corresponding timestamp:", best_split_timestamp_flow)

print("Best split point for sediment concentration:", best_split_point_sediment)
print("Corresponding timestamp:", best_split_timestamp_sediment)
...
# 水位效果图
plt.subplot(3, 1, 1)
plt.plot(timestamps, water_level[:best_split_point_water], 'b')
plt.plot(timestamps, water_level[best_split_point_water:], 'r')
plt.axvline(x=best_split_timestamp_water, color='k', linestyle='--')
plt.xlabel('Time')
...
# 流量效果图
plt.subplot(3, 1, 2)
plt.plot(timestamps, flow_rate[:best_split_point_flow], 'b')
plt.plot(timestamps, flow_rate[best_split_point_flow:], 'r')
plt.axvline(x=best_split_timestamp_flow, color='k', linestyle='--')
plt.xlabel('Time')
...
# 含沙量效果图
plt.subplot(3, 1, 3)
plt.plot(timestamps, sediment_concentration[:best_split_point_sediment], 'b')
plt.plot(timestamps, sediment_concentration[best_split_point_sediment:], 'r')
plt.axvline(x=best_split_timestamp_sediment, color='k', linestyle='--')
plt.xlabel('Time')
